...
async function fetchRemoteInfo(session) {
  const command = `
    set -e;
    DISTRO=$( (cat /etc/os-release 2>/dev/null || true) | awk -F '=' '/^PRETTY_NAME/{gsub("\"","",$2);print $2}' );
    KERNEL=$(uname -sr);
    UPTIME=$(cat /proc/uptime | awk '{printf("%0.0f", $1)}');
    USERCOUNT=$(who | wc -l);
    IP=$(hostname -I 2>/dev/null | awk '{print $1}');
    echo "{\"distro\":\"\${DISTRO:-unknown}\",\"kernel\":\"\${KERNEL}\",\"uptimeSeconds\":\${UPTIME:-0},\"users\":\${USERCOUNT:-0},\"ip\":\"\${IP:-n/a}\"}";
  `;
  return executeRemote(session, command);
}

async function fetchProcesses(session) {
  const command = "ps aux --sort=-%mem | head -n 15";
  return executeRemote(session, command);
}

async function fetchPorts(session) {
  const command = "ss -tulpn | head -n 20";
  return executeRemote(session, command);
}

async function tailFile(session, filePath, lines = 50) {
  const safePath = sanitizeRemotePath(filePath);
  const command = `tail -n ${Number(lines) || 50} '${safePath}'`;
  return executeRemote(session, command);
}

async function runArbitrary(session, remoteCommand) {
  const safe = remoteCommand || '';
  return executeRemote(session, safe);
}

async function fetchTree(session, basePath, depth = 2) {
  const safePath = sanitizeRemotePath(basePath);
  const d = Number(depth) || 2;
  const command = `cd '${safePath}' && find . -maxdepth ${d} -print`;
  return executeRemote(session, command);
}
...
function handleApi(req, res) {
  ...
  if (req.method === 'GET' && pathname.startsWith('/api/session/') && pathname.endsWith('/ping')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    executeRemote(session, 'echo pong $(date +%s%3N)')
      .then(output => jsonResponse(res, 200, { output }))
      .catch(err => jsonResponse(res, 500, { error: err.message }));
    return;
  }

  if (req.method === 'GET' && pathname.startsWith('/api/session/') && pathname.endsWith('/info')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    fetchRemoteInfo(session)
  ...
  if (req.method === 'GET' && pathname.startsWith('/api/session/') && pathname.endsWith('/processes')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    fetchProcesses(session)
      .then(output => jsonResponse(res, 200, { output }))
      .catch(err => jsonResponse(res, 500, { error: err.message }));
    return;
  }

  if (req.method === 'GET' && pathname.startsWith('/api/session/') && pathname.endsWith('/ports')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    fetchPorts(session)
  ...
  if (req.method === 'POST' && pathname.startsWith('/api/session/') && pathname.endsWith('/tail')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    parseBody(req)
      .then(body => {
        ...
        return tailFile(session, body.path, body.lines);
      })
      .then(output => jsonResponse(res, 200, { output }))
      .catch(err => jsonResponse(res, 500, { error: err.message }));
    return;
  ...
  if (req.method === 'POST' && pathname.startsWith('/api/session/') && pathname.endsWith('/run')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    parseBody(req)
      .then(body => {
        ...
        return runArbitrary(session, body.command);
      })
      .then(output => jsonResponse(res, 200, { output }))
      .catch(err => jsonResponse(res, 500, { error: err.message }));
    return;
  ...
  if (req.method === 'GET' && pathname.startsWith('/api/session/') && pathname.endsWith('/tree')) {
    const id = pathname.split('/')[3];
    const session = ensureSession(res, id);
    if (!session) return;
    fetchTree(session, parsed.query.path || '.', parsed.query.depth || 2)
      .then(output => jsonResponse(res, 200, { output }))
      .catch(err => jsonResponse(res, 500, { error: err.message }));
    return;
  }
